...
class PathNode:
    ...
    def __init__(self):
        ...
        # 0 -> 1
        # =====================
        self.p01_x = np.linspace(-X_CENTER, X_CENTER, pts * 2)
        self.p01_y = np.zeros_like(self.p01_x)

        # =====================
        # 1 -> 2
        # =====================
        x1, y1 = self.arc(X_RIGHT, Y_BOTTOM + ARC_R, ARC_R, th_br_out)
        x2, y2 = self.arc(X_RIGHT, Y_BOTTOM + ARC_R, ARC_R, th_br_in)

        self.p12_x = np.concatenate([
            np.linspace(X_CENTER, X_RIGHT, pts),
            x1, x2,
            np.linspace(X_RIGHT, X_CENTER, pts)
        ])

        self.p12_y = np.concatenate([
            Y_BOTTOM * np.ones(pts),
            y1, y2,
            Y_MID * np.ones(pts)
        ])
        ...
        # 2 -> 3
        # =====================
        x1, y1 = self.arc(X_LEFT, Y_TOP - ARC_R, ARC_R, th_tl_out)
        x2, y2 = self.arc(X_LEFT, Y_TOP - ARC_R, ARC_R, th_tl_in)

        self.p23_x = np.concatenate([
            np.linspace(X_CENTER, X_LEFT, pts),
            x1, x2,
            np.linspace(X_LEFT, X_CENTER, pts)
        ])
        ...
        # 3 -> 2
        # =====================
        x1, y1 = self.arc(X_RIGHT, Y_TOP - ARC_R, ARC_R, th_tr_in)
        x2, y2 = self.arc(X_RIGHT, Y_TOP - ARC_R, ARC_R, th_tr_out)
        ...
        # 2 -> 1
        # =====================
        x1, y1 = self.arc(X_LEFT, Y_BOTTOM + ARC_R, ARC_R, th_bl_in)
        x2, y2 = self.arc(X_LEFT, Y_BOTTOM + ARC_R, ARC_R, th_bl_out)
